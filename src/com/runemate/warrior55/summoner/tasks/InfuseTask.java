        ...
        if (MakeXInterface.isOpen()) {
            ...
            if (obelisk != null && Players.getLocal().distanceTo(obelisk) < 10) {
                ...
                if (pathToObelisk == null || pathToObelisk.getNext() == null) {
                    //pathToObelisk = new Web().getPathBuilder().useTeleports(false).buildTo(new Coordinate(2930, 3448));
                    pathToObelisk = PredefinedPath.create(pathToObelisk2);
                }

                if (pathToObelisk != null) {
                    pathToObelisk.step(false);
